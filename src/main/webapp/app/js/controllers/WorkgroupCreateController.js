...
define(['angular', 'controllers'], function(angular, controllers) {
    ...
    controllers.controller('WorkgroupCreateController',
        ['$scope', '$http',
            function($scope, $http) {
                $scope.master = {};
                var allpermissions = [],
                    baseUrl = '/JerseyTest/rest/ldap/permission/all';
                ...
                function search() {
                    $http
                        ...
                        function(data, status) {
                            ...
                            allpermissions = data;
                            $scope.errorMsg =
                                'Success fetching permissions!!' + status;
                        })
                        .error(function(data, status) {
                            $scope.errorMsg =
                                'Permissions Fetch Failed' + status;
                            $scope.permissions = [];
                ...
                $scope.create = function(workgroup) {
                    ...
                    $http
                        .post('/JerseyTest/rest/ldap/profile/create',
                            $scope.master)
                        .success(
                        function(data, status) {
                            $scope.errorMsg =
                                'Successfully created Profile ' + status;
                        })
                        .error(function(data, status) {
                            $scope.errorMsg =
                                'No Doughnut for you :) ' + status;
                        });
                };
                ...
                $scope.addToWorkGroupPermissions =
                    function(permissionsToBeAdded) {
                        var i = 0;
                        if (permissionsToBeAdded) {
                            $scope.workgroupPermissions.push.apply(
                                $scope.workgroupPermissions,
                                permissionsToBeAdded
                            );
                        permissionsToBeAdded.forEach(
                            function(item) {
                ...
                $scope.removeFromWorkGroupPermissions =
                    function(permissionsToBeRemoved) {
                    var i = 0;
                    if (permissionsToBeRemoved) {
                        $scope.permissions.push.apply(
                            $scope.permissions,
                            permissionsToBeRemoved
                        );
